...
func SentryFiber(options Options) fiber.Handler {
...
func (h *handler) handle(ctx *fiber.Ctx) error {
	...
	transaction.SetTag("http.method", ctx.Method())
	transaction.SetTag("http.route", ctx.Route().Path)
	transaction.SetTag("http.url", ctx.OriginalURL())
	transaction.SetTag("user_agent", ctx.Get("User-Agent"))
	transaction.SetData("http.request.body_size", len(ctx.Body()))
	defer func() {
